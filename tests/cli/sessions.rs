...
#[test]
fn server_stop_then_restart_restores_local_hostname_reported_cwd() {
    let base = unique_test_dir();
    let config_home = base.join("config");
    let runtime_dir = base.join("runtime");
    let socket_path = runtime_dir.join("herdr.sock");
    let client_socket = runtime_dir.join("herdr-client.sock");
    let start_cwd = base.join("start");
    let reported_cwd = base.join("reported");
    let restored_pwd = base.join("restored-pwd.txt");
    fs::create_dir_all(&start_cwd).unwrap();
    fs::create_dir_all(&reported_cwd).unwrap();
    ...
    assert!(hostname.status.success(), "hostname command should succeed");
    let hostname = String::from_utf8(hostname.stdout).unwrap();
    let hostname = hostname.trim();
    assert!(!hostname.is_empty(), "hostname should not be empty");
    let report_cwd = format!(
    ...
    assert!(
        wait_until(Duration::from_secs(3), Duration::from_millis(25), || {
            let pane = run_cli_json(&socket_path, &["pane", "get", &pane_id]);
            pane["result"]["pane"]["cwd"] == reported_cwd.to_string_lossy().as_ref()
        }),
        "pane should accept OSC 7 cwd reports for the local hostname"
    );
    ...
    let workspace_id = workspaces["result"]["workspaces"]
        ...
        .expect("workspace.list should return workspaces")
        .iter()
        .find(|workspace| workspace["label"] == "local-hostname-cwd-restart")
        .and_then(|workspace| workspace["workspace_id"].as_str())
        .expect("restored workspace should exist")
        .to_string();
    let panes = run_cli_json(
    ...
    assert_eq!(
        ...
        "restored pane should start in the local hostname reported cwd"
    );
    ...
    assert!(
        wait_until(Duration::from_secs(3), Duration::from_millis(25), || {
            fs::read_to_string(&restored_pwd)
                .is_ok_and(|pwd| pwd.trim() == reported_cwd.to_string_lossy())
        }),
        "restored shell should actually run in the local hostname reported cwd"
    );
