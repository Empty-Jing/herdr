...
| セッション | `session.snapshot`、`session.report_metadata` |
...
`session.snapshot` は、独自のローカルランタイムキャッシュを持つクライアント向けに、一度限りのブートストラップスナップショットを返します。レスポンスには、バージョン/プロトコルメタデータ、フォーカス中のワークスペース/タブ/ペイン id、セッショントークン、ワークスペース、タブ、ペイン、タブレイアウト、エージェントの各レコードが含まれます。これは購読ではありません。取得後はリソースイベントを購読し、そのイベントでローカルキャッシュを更新してください。再接続後やキャッシュが古い可能性がある場合は、`session.snapshot` を再度呼び出します。ワークスペースレコードには、関連付けられた worktree の出自情報も含まれます。リポジトリ全体の worktree 検出には引き続き `worktree.list` を使います。
...
セッショントークンも同じパッチ規約を使い、`session_id` はありません。接続済みソケットがセッションを選択しています。`session.report_metadata` には `source`、`tokens`、任意の `seq` と `ttl_ms` を渡します。`session.snapshot` は結果の `tokens` マップを公開します。変更と TTL の失効は、現在の完全なトークンマップを持つ `session.metadata_updated` を発行し、プラグインイベントフックは実行しません。
...
短寿命のメタデータには `ttl_ms` を使ってください。`1` から `86400000` ミリ秒の間でなければなりません。置き換え・クリア・対象のペイン、ワークスペース、またはセッションの終了まで残るべきメタデータでは省略します。表示フィールドは従来どおりソース単位で失効し、呼び出しで更新した各トークンには個別の期限が設定されます。トークンメタデータはサーバー再起動後に復元されません。

フックが順不同で更新を送る可能性がある場合は `seq` を使ってください。同じ `source` について、最後に受理されたシーケンス以下のシーケンス番号を持つ報告は、API には受理されますが無視されます。ペイン、ワークスペース、またはセッションが存続する間、シーケンス付きトークン報告を送れる異なるソースは最大 32 個です。クリアや失効でもそのソース枠は解放されません。
...
セッションイベント購読には `session.metadata_updated` があります。現在の完全なトークンマップでトークン変更と TTL 失効を報告し、プラグインイベントフックは実行しません。
