...
### オプションの OpenAI クォータカード

独立したオプトインの Pi クォータ拡張をインストールします:

```bash
herdr integration install pi-quota
```

Herdr は `~/.pi/agent/extensions/herdr-openai-quota.ts`、または `PI_CODING_AGENT_DIR` から解決された拡張ディレクトリに書き込みます。インストール後、既存の Pi セッションで `/reload` を実行するか、Pi を再起動してください。`herdr integration uninstall pi-quota` はクォータ拡張だけを削除します。

対象の Pi ペインがアクティブな間だけ、この拡張は Pi の拡張 API を通じて既存の `openai-codex` OAuth ログインを解決し、`https://chatgpt.com/backend-api/wham/usage` をリクエストして、正規化されたクォータメタデータだけを Herdr に報告します。access token、account id、生のレスポンスは報告しません。Pi と OpenCode のクォータインテグレーションは互いの新しいメタデータを再利用するため、2 つのエージェントを切り替えても重複したリモートリクエストは発生しません。

アクティブな Pi と OpenCode のペインは同じクォータカードを共有します。API キーログインでは ChatGPT サブスクリプションのクォータを取得できず、ChatGPT usage endpoint は OpenAI の公開 API ではないため、Herdr とは独立して変更される可能性があります。
...
OpenAI クォータインテグレーションはデフォルトで無効であり、設定画面から推奨インテグレーションをインストールしても含まれません。明示的に有効化します:

```bash
herdr integration install opencode-quota
```

このコマンドは別のプラグインを `~/.config/opencode/plugins/herdr-openai-quota.js` に書き込み、標準の OpenCode ライフサイクルプラグインをインストールまたは変更しません。新しいプラグインを読み込むには、インストール後に OpenCode を再起動してください。`herdr integration uninstall opencode-quota` はクォータプラグインだけを削除します。

このプラグインは OpenCode の公開 auth loader を使い、OpenCode が管理している既存の OpenAI OAuth ログインにアクセスします。対象の OpenCode ペインがアクティブな間だけ `https://chatgpt.com/backend-api/wham/usage` をリクエストし、正規化した状態、プラン、ウィンドウ期間、残量率、リセット時刻を Herdr のローカルペインメタデータへ報告します。access token、account id、refresh token、生のレスポンスは Herdr に報告されません。API キーログインでは ChatGPT サブスクリプションのクォータを取得できません。OAuth access が期限切れの場合、カードは stale 状態を表示し、プラグイン自身で認証情報を更新せずに OpenCode によるログイン更新を待ちます。

サイドバーカードは返された期間から各クォータウィンドウのラベルを動的に生成し、固定の 5 時間または週間ウィンドウを仮定しません。クォータウィンドウの下には主ウィンドウのリセットまでの残り時間を表示し、必要に応じて副ウィンドウへフォールバックします。ChatGPT usage endpoint は OpenAI の公開 API ではなく、Herdr とは独立して変更される可能性があります。
