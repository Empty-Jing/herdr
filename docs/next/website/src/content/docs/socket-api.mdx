...
| Session | `session.snapshot`, `session.report_metadata` |
...
focused workspace/tab/pane ids, session tokens, workspace records, tab records, pane records,
...
Session tokens use the same patch contract without a `session_id`; the connected socket already selects the session. Call `session.report_metadata` with `source`, `tokens`, and optional `seq` and `ttl_ms`. `session.snapshot` exposes the resulting `tokens` map. Changes and TTL expiry emit `session.metadata_updated` with the complete current token map, without invoking plugin event hooks.
...
Use `ttl_ms` for short-lived metadata. It must be between `1` and `86400000` milliseconds. Omit `ttl_ms` for metadata that should stay until replaced, cleared, or its pane/workspace/session ends. Presentation fields retain their existing source-scoped expiry behavior; each token updated by the call receives its own deadline. Token metadata is not restored after a server restart.

Use `seq` when a hook may send updates out of order. For the same `source`, reports with a sequence number less than or equal to the last accepted sequence are accepted by the API but ignored. A pane, workspace, or session accepts sequenced token reports from at most 32 distinct sources during its lifetime; clearing or expiry does not release those source slots.
...
Session event subscriptions include `session.metadata_updated`, which reports token changes and TTL expiry with the complete current token map without invoking plugin event hooks.
