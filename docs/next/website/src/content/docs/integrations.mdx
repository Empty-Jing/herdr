...
herdr integration install pi-quota
...
herdr integration install opencode-quota
...
herdr integration uninstall pi-quota
...
herdr integration uninstall opencode-quota
...
### Optional OpenAI quota card

Install the separate, opt-in Pi quota extension:

```bash
herdr integration install pi-quota
```

Herdr writes it to `~/.pi/agent/extensions/herdr-openai-quota.ts`, or the resolved `PI_CODING_AGENT_DIR` extension directory. Run `/reload` in an existing Pi session, or restart Pi, after installation. `herdr integration uninstall pi-quota` removes only the quota extension.

While its Pi pane is active, the extension resolves the existing `openai-codex` OAuth login through Pi's extension API, requests `https://chatgpt.com/backend-api/wham/usage`, and reports only normalized quota metadata to Herdr. It never reports the access token, account id, or raw response. The Pi and OpenCode quota integrations reuse fresh metadata from each other, so switching between the two agents does not make duplicate remote requests.

The quota card is shared by active Pi and OpenCode panes. API-key logins do not expose ChatGPT subscription quota, and the ChatGPT usage endpoint is not a public OpenAI API and may change independently of Herdr.
...
The OpenAI quota integration is disabled by default and is not included when the settings screen installs recommended integrations. Enable it explicitly:

```bash
herdr integration install opencode-quota
```

This writes a separate plugin to `~/.config/opencode/plugins/herdr-openai-quota.js`; it does not install or modify the standard OpenCode lifecycle plugin. Restart OpenCode after installing so it loads the new plugin. Remove only the quota plugin with `herdr integration uninstall opencode-quota`.

The plugin uses OpenCode's public auth loader to access the OpenAI OAuth login already managed by OpenCode. Only while that OpenCode pane is active, it requests `https://chatgpt.com/backend-api/wham/usage` and reports normalized status, plan, window duration, remaining percentage, and reset time to Herdr's local pane metadata. The access token, account id, refresh token, and raw response are never reported to Herdr. API-key logins do not expose ChatGPT subscription quota. If the OAuth access has expired, the card shows stale data status and waits for OpenCode to refresh its own login instead of refreshing credentials itself.

The sidebar card labels each available quota window from its returned duration rather than assuming fixed five-hour or weekly windows. Below the quota windows, it shows the primary window's reset countdown and falls back to the secondary window when needed. The ChatGPT usage endpoint is not a public OpenAI API and may change independently of Herdr.
