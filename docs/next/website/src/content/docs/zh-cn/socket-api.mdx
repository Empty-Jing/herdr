...
| 会话 | `session.snapshot`、`session.report_metadata` |
...
`session.snapshot` 为维护本地运行时缓存的客户端返回一次性引导快照。响应包含版本/协议元数据、当前聚焦的工作区/标签页/窗格 id、会话令牌、工作区记录、标签页记录、窗格记录、标签页布局快照和智能体记录。它不是订阅;读取后应订阅资源事件,并根据事件更新本地缓存。重新连接后或本地缓存可能已过期时,再次调用 `session.snapshot`。工作区记录也包含关联 worktree 的来源信息。完整的仓库 worktree 发现仍使用 `worktree.list`。
...
会话令牌使用相同的补丁约定,且不带 `session_id`;已连接的 socket 已经选定会话。调用 `session.report_metadata` 时传入 `source`、`tokens` 以及可选的 `seq` 和 `ttl_ms`。`session.snapshot` 公开生成的 `tokens` 映射。变更和 TTL 到期会通过 `session.metadata_updated` 发出当前完整令牌映射,且不会调用插件事件钩子。
...
短期元数据用 `ttl_ms`。取值必须在 `1` 到 `86400000` 毫秒之间。想让元数据保留到被替换、清除或其窗格、工作区或会话结束时,省略 `ttl_ms`。展示字段继续按来源过期;该次调用更新的每个令牌都有独立期限。令牌元数据不会在服务器重启后恢复。

钩子可能乱序发送更新时,使用 `seq`。对同一 `source`,序号小于等于最后接受序号的上报会被 API 接受但忽略。每个窗格、工作区或会话在其生命周期内最多接受来自 32 个不同来源的带序号令牌上报;清除或过期不会释放这些来源名额。
...
会话事件订阅包括 `session.metadata_updated`,它使用当前完整令牌映射报告令牌变更和 TTL 到期,且不会调用插件事件钩子。
