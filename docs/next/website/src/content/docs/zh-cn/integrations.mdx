...
安装独立且需要显式启用的 Pi 额度扩展:

```bash
herdr integration install pi-quota
```

Herdr 会将它写入 `~/.pi/agent/extensions/herdr-openai-quota.ts`,或 `PI_CODING_AGENT_DIR` 解析出的扩展目录。安装后,在已有 Pi 会话中运行 `/reload`,或重启 Pi。`herdr integration uninstall pi-quota` 只删除额度扩展。

只有对应 Pi 窗格处于活动状态时,该扩展才会通过 Pi 的扩展 API 解析现有 `openai-codex` OAuth 登录,请求 `https://chatgpt.com/backend-api/wham/usage`,并只向 Herdr 上报归一化额度元数据。它不会上报 access token、account id 或原始响应。Pi 和 OpenCode 额度集成会互相复用新鲜元数据,所以在两个智能体间切换不会产生重复远程请求。

活动的 Pi 和 OpenCode 窗格共用同一张额度卡片。API Key 登录不提供 ChatGPT 订阅额度,并且 ChatGPT usage endpoint 不是 OpenAI 的公开 API,可能独立于 Herdr 发生变化。
...
OpenAI 额度集成默认关闭,设置界面安装推荐集成时也不会包含它。必须显式启用:

```bash
herdr integration install opencode-quota
```

该命令会把独立插件写入 `~/.config/opencode/plugins/herdr-openai-quota.js`,不会安装或修改标准 OpenCode 生命周期插件。安装后需重启 OpenCode,以加载新插件。运行 `herdr integration uninstall opencode-quota` 只会删除额度插件。

该插件通过 OpenCode 的公开 auth loader 使用由 OpenCode 管理的现有 OpenAI OAuth 登录。只有对应 OpenCode 窗格处于活动状态时,插件才会请求 `https://chatgpt.com/backend-api/wham/usage`,并向 Herdr 的本地窗格元数据上报归一化后的状态、套餐、窗口时长、剩余百分比和重置时间。access token、account id、refresh token 和原始响应都不会上报给 Herdr。API Key 登录不提供 ChatGPT 订阅额度。OAuth access 过期时,卡片会显示 stale 状态并等待 OpenCode 自行刷新登录,插件不会自行刷新凭据。

侧栏卡片根据返回的窗口时长动态生成每个额度窗口的标签,不会假定固定的五小时或每周窗口。额度窗口下方会显示主窗口的重置倒计时,必要时回退到次级窗口。ChatGPT usage endpoint 不是 OpenAI 的公开 API,可能独立于 Herdr 发生变化。
